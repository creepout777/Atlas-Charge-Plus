    ...
    <>
      ...
      {!session && (
        <>
          ...
          <NavLink to="/connectors" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <Layers size={15} /> Hardware
          </NavLink>
          <NavLink to="/reviews" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
      ...
      {session && role === 'CLIENT' && (
        <>
          <NavLink to="/" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <Navigation size={15} /> Dispatch Map
            {activeOrder && <span className="status-dot emerald pulse" style={{ width: '6px', height: '6px' }} />}
          </NavLink>
          <NavLink to="/vehicles" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <Car size={15} /> Garage ({vehicles.length})
          </NavLink>
          <NavLink to="/history" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
      ...
      {session && role === 'DRIVER' && (
        <>
          <NavLink to="/driver" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
      ...
      {session && role === 'FLEET_DISPATCHER' && (
        <>
          <NavLink to="/admin" onClick={closeMenu} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
    ...
    <>
      <header className="top-nav">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {!session ? (
            <NavLink to="/login" onClick={closeMenu} className="btn-emerald" style={{ padding: '8px 18px', fontSize: '13px', width: 'auto' }}>
      ...
      <div className={`mobile-nav-drawer ${mobileMenuOpen ? 'active' : ''}`}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid var(--border-subtle)' }}>
          <span style={{ fontWeight: 800, fontSize: '15px' }}>Navigation</span>
