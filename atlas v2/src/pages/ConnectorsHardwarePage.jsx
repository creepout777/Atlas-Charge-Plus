...
import { Layers, Zap, Cpu, Plus, Edit3, Trash2, Lock, Archive, CheckCircle2, RotateCcw } from 'lucide-react';
...
export default function ConnectorsHardwarePage() {
  ...
  // Role Security Check: ONLY SuperAdmin has access to edit/create/delete hardware specs
  ...
  return (
    <div style={{ maxWidth: '960px', margin: '32px auto', padding: '0 20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
            Physical connector protocols & mounted mobile dispensing assemblies · Supports Soft-Archiving
          </div>
        </div>
        {isSuperAdmin && (
      ...
      {/* --- MODAL: ADD CONNECTOR STANDARD (SuperAdmin) --- */}
      <Modal isOpen={showAddConnModal} onClose={() => setShowAddConnModal(false)} title="Register New Connector Standard (SuperAdmin)">
        <form onSubmit={handleAddConnSubmit}>
          <div style={{ marginBottom: '12px' }}>
            <label style={{ fontSize: '12px', fontWeight: 800, display: 'block', marginBottom: '4px' }}>Standard Code (Auto-Locked)</label>
          ...
          <button type="submit" className="btn-emerald">
            Save Connector Type to Database
          </button>
        </form>
      </Modal>

      {/* --- MODAL: EDIT CONNECTOR STANDARD (SuperAdmin) --- */}
      <Modal isOpen={showEditConnModal} onClose={() => setShowEditConnModal(false)} title="Edit Connector Standard (SuperAdmin)">
      ...
      {/* --- MODAL: MOUNT CONNECTOR TO TRUCK (SuperAdmin) --- */}
      <Modal isOpen={showAddTruckConnModal} onClose={() => setShowAddTruckConnModal(false)} title="Mount Dispenser Connector to Truck (SuperAdmin)">
        <form onSubmit={handleAddTruckConnSubmit}>
          ...
          <button type="submit" className="btn-emerald">
            Mount Connector Assembly
          </button>
        </form>
      </Modal>

      {/* --- MODAL: EDIT MOUNTED CONNECTOR (SuperAdmin) --- */}
      <Modal isOpen={showEditTruckConnModal} onClose={() => setShowEditTruckConnModal(false)} title="Edit Mounted Connector Assembly (SuperAdmin)">
